...
//FLASH写入数据
void FLASH_W(u32 add,u16 dat1,u16 dat2)//参数1：32位FLASH地址；参数2：16位数据；参数3:16位数据（多数据在这里增加16位形参即可）
{
//	 RCC_HSICmd(ENABLE); //读写flash需要打开内部或外部高速时钟，如果前面已经有程序已经打开时钟，则这里可以注释掉
     ...
     FLASH_ProgramHalfWord(add,dat1); //从指定页的addr地址开始写入dat1数据
	 FLASH_ProgramHalfWord(add, dat2);//从指定页的addr地址开始写入dat2数据
