...
volatile double distance_cm = 0;//当前距离
...
//	{
//		ultick_5ms = 0;
		GetDistanceDelay();       //延时方式读取距离值
//	}
}


/*******************************************************************************
* 函 数 名         : GetDistanceDelay
* 函数功能		     : 延时的方式读取距离值
* 输    入         : 无
* 输    出         : 无
*                             黎孟度心血之作                                   *
...
//				distance_cm = 0;//失败后就后退
		...
		TIM_Cmd(TIM4, DISABLE);//关闭定时器
		distance_cm = (double)(((dis_count) * 34.0)/2000.0);//声速340m/s
		printf("%lf cm \r\n", distance_cm);	
		status = 0;//准备下次发送		
}
#endif
