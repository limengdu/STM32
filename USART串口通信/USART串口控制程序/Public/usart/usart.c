...
{ 
    ...
    //USART1_TX   PA.9 (STM32F1系列的引脚都是不变的)
    GPIO_InitStructure.GPIO_Pin = GPIO_Pin_9; //PA.9
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;	//复用推挽输出（输出需要使用复用推挽）
    GPIO_Init(GPIOA, &GPIO_InitStructure);  
    //USART1_RX	  PA.10
    GPIO_InitStructure.GPIO_Pin = GPIO_Pin_10;
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING;//浮空输入（接收端只需要浮空即可）
    GPIO_Init(GPIOA, &GPIO_InitStructure); 
    //Usart1 NVIC 配置
...
void USART1_IRQHandler(void)//串口1中断服务程序（固定的函数名不能修改）想要进入中断首先要记得开启中断！！
{ 	                        //一般中断服务函数要写在stm32f10x_it.c中，这里为了方便一次性看完代码，就先写在这
	u8 a;
